...
### We are just going to use the function colnames and then refer to the column we  are  interested in.
...
DEM$VISIT <- as.character(DEM$VISIT)

LU_WORD <- full_join(LU, WORD, by = c("SUBJ", "VISIT"))
df <- DEM %>% left_join(LU_WORD, by = c("SUBJ", "VISIT"))
...
visit_1 <- df %>% 
  ...
  select(SUBJ, ADOS1, MullenRaw1, ExpressiveLangRaw1, Socialization1)

df <- df %>% left_join(visit_1, by = "SUBJ")
...
dfa <- df
dfa$SUBJ <- as.factor(dfa$SUBJ)
dfa$SUBJ <- as.numeric(dfa$SUBJ)
dfa$SUBJ <- as.factor(dfa$SUBJ)

# Renaming gender
dfa$Gender <- as.factor(dfa$Gender)
dfa$Gender <- fct_recode(dfa$Gender, "M" = "1", "F" = "2")

# Renaming diagnosis
dfa$Diagnosis <- fct_recode(dfa$Diagnosis, "ASD" = "A", "TD" = "B")

# Export as csv
write.csv(dfa, "p1_df.csv")
```
...
```{r}
# 1
df %>% filter(CHI_MLU > 2.7)

# 2
df %>% filter(VISIT == 1 & CHI_MLU < 1.5)
...
```{r}
# 1
df %>% filter(VISIT == 6) %>% arrange(desc(tokens_CHI))
# TC
...
select_2 <- dfa %>% 
  select(SUBJ, SUBJ)
# Nothing happens, it only selects the variable once
```
...
```{r}
# 1
df <- df %>% mutate(oa_mean_MLU = mean(tokens_CHI, na.rm = T))

# 2
df <- df %>% group_by(SUBJ) %>% mutate(mean_MLU = mean(tokens_CHI, na.rm = T))
