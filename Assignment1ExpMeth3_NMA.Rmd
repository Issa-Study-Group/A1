...
setwd("/Users/Nikol/Documents/R/Data")

demographic_data <- read.csv("demo_train.csv")

len_utterance_data <- read.csv("LU_train.csv")
...
view(len_utterance_data)
view(word_data)

#Variables with different names are; SUBJ (Child.ID in demographic data) 

# Changing the demographic data Child ID to SUBJ so it will match the other dataframes

library(data.table)
setnames(demographic_data, "Child.ID", "SUBJ")

#Chaning to visit to all caps in demographic data to match the other dataframes

setnames(demographic_data, "Visit", "VISIT")
...
#Extracting the digit from a string using str_extract, so that the way "VISIT" is reported matches in all dataframes

library(stringr)

len_utterance_data$VISIT <- str_extract(len_utterance_data$VISIT, "[[:digit:]]+")
word_data$VISIT <- str_extract(word_data$VISIT, "[[:digit:]]+")

len_utterance_data$VISIT <- as.numeric(len_utterance_data$VISIT)
word_data$VSIT <- as.numeric(word_data$VISIT)
...
#Removing all periods in the SUBJ variable in all dataframes

demographic_data$SUBJ <- gsub("\\.", "", demographic_data$SUBJ)
len_utterance_data$SUBJ <- gsub("\\.", "", len_utterance_data$SUBJ)
word_data$SUBJ <- gsub("\\.", "", word_data$SUBJ)
...
#Selecting relevant variables from the first dataset

demographic_data_new <- select(demographic_data, SUBJ, VISIT, Diagnosis, Ethnicity, Gender, Age, ADOS, MullenRaw, ExpressiveLangRaw, Socialization) 

len_utterance_data_new <- select(len_utterance_data, MOT_MLU, CHI_MLU)

word_data_new <- select(word_data, types_MOT, types_CHI, tokens_MOT, tokens_CHI)
...
combined_data <- merge.data.frame(demographic_data_new, len_utterance_data_new)

combined_data <- merge.data.frame(combined_data, word_data_new)
